...
**NOTE**: 由于安全原因，不建议对外公网开放 IPV6 服务。本文开启 IPV 6 主要是为了后文的内网穿透，此处为可选。
...
首先我们创建一个新的邮箱账号（对于国内用户建议是 outlook），例如我的是 public.locez@outlook.com EMAIL ，然后对这个账号进行安全配置，比如验证邮箱，两步验证等，后续是要将这个邮箱账号发送给你信任的朋友直接使用的，因此必要的配置还是要做，要始终将邮箱的所有权掌握在自己手中。
...
```
然后在 tailscale 的设置中添加 ACL 配置，在 ACL 配置中添加一个规则，允许所有的设备访问，然后将这个规则应用到所有的设备。
...
**NOTE**：在使用 QNAP 的过程中，我尝试使用了 IPV6，为了以后能有机会暴露出服务，碰到 IPV6 这部分内容可以选择性跳过，添加 ipv6 选项即可
